...
import { resubmitCourse } from '../../services/instructorService'
import { useNavigate, useParams } from 'react-router-dom'
import { useInstructorCourse } from '../../hooks/useInstructorCourses'


const ResubmitCourseForm = ({ course }) => {
    ...
    const mutation = useMutation({
        mutationFn: ({ courseId, courseData }) => resubmitCourse(courseId, courseData),
        onSuccess: async () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        mutation.mutate({ courseId: course._id, courseData: formData })
    }
    ...
    return (
        <main className="flex items-center justify-center min-h-screen bg-gray-900 py-12 px-4">
            ...
            <div className="w-full max-w-2xl bg-gray-800 rounded-lg shadow-lg p-8 space-y-6">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-white"> Resubmit Course </h1>
                    <p className="text-gray-400 mt-2">
                        Update the current course details and resubmit for review.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex gap-4 pt-6">
                        ...
                        >
                            {mutation.isPending ? "Resubmitting..." : "Resubmit Course"}
                        </button>
                        <button
                            type="button"
...
const ResubmitCourse = () => {
    const navigate = useNavigate()
    const { courseId } = useParams()
    const { courses, isLoading, error } = useInstructorCourse()
    const currentCourse = courses.find((item) => item._id === courseId)

    if (isLoading) {
        return (
            <main className="flex items-center justify-center min-h-screen bg-gray-900 py-12 px-4 text-gray-300">
    ...
    if (error || !currentCourse) {
        return (
            <main className="flex items-center justify-center min-h-screen bg-gray-900 py-12 px-4 text-gray-300">
                <div className="max-w-md rounded-lg border border-red-700 bg-gray-800 p-6 text-center">
                    <h1 className="text-xl font-semibold text-white">Course not found</h1>
                    <p className="mt-2 text-sm text-gray-400">The rejected course could not be loaded. Please go back and open it again from the rejected courses list.</p>
                    <button
                        type="button"
                        onClick={() => navigate("/instructor/dashboard")}
    ...
    return <ResubmitCourseForm course={currentCourse} />
}

export default ResubmitCourse
